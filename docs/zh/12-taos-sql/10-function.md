...
- INTERP 根据 EVERY(time_unit) 字段来确定输出时间范围内的结果条数，即从 timestamp1 开始每隔固定长度的时间（time_unit 值）进行插值，支持的时间单位 time_unit 为: 1a(毫秒)，1s(秒)，1m(分)，1h(小时)，1d(天), 1w(周)。
- INTERP 根据 FILL 字段来决定在每个符合输出条件的时刻如何进行插值。FILL 子句的使用方式请参考 [FILL 子句](./distinguished/#fill-子句)
